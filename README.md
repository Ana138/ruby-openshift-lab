...
If you'd like to install it, follow [these directions](https://github.com/Ana138/ruby-openshift-lab/master/README.md#installation).  
...
3. A build should start immediately.  To run another build, run:
		...
		$ oc start-build ruby-openshift-lab

4. Once the build is running, watch your build progress:  

		$ oc logs build/ruby-openshift-lab-1
...
5. Wait for ruby-ex pods to start up (this can take a few minutes):  
	...
	Sample output:  
		...
		ruby-openshift-lab-1-build    0/1       ExitCode:0   0          2m
		ruby-openshift-lab-1-deploy   1/1       Running      0          25s
		ruby-openshift-lab-1-hrek2    1/1       Running      0          17s
...
6. Check the IP and port the ruby-ex service is running on:  
	...
	Sample output:  
		...
		ruby-ex   172.30.97.209   <none>        8080/TCP   deploymentconfig=ruby-openshift-lab   2m
